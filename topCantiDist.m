...
%%%% CODE MODIFIED BY F&L - 2013  %%%
function topCantiDist(nelx,nely,volfrac,penal,rmin);
global nelx  nely volfrac penal rmin cstory loop th x passive
% INITIALIZE
x(1:nely,1:nelx) = volfrac;  % def of initial density
loop = 0; 
change = 1.;
cstory = 0;
% START ITERATION
...
% F((2*(nely+1)*(nelx))+1:2:2*(nelx+1)*(nely+1)-1,1) = 0.1;
F((2:2*(nely+1):2*(nely+1)*(nelx)+2),1) = -0.5;   % Distributed load on upper face
